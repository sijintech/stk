...
draw_handle = bpy.types.SpaceSTK.draw_handler_add(observe, (), "WINDOW", "POST_PIXEL")
...
def step():
    ...
    try:
        ...
        areas = [a for a in window.screen.areas if a.type == "STK"]
        ...
        if stage == 0:
            ...
            tx, ty = int(area.x+w*.72+95), int(area.y+16+(row-3)/2)
            event(window,"MOUSEMOVE","NOTHING",x=tx,y=ty)
            event(window,"LEFTMOUSE",x=tx,y=ty)
            event(window,"LEFTMOUSE","RELEASE",x=tx,y=ty)
        elif stage == 4:
            ...
            assert any(c["kind"]=="chat" and c["payload"]["content"]=="中文测试" for c in commands), "Unicode did not pass through native text control and command queue"
