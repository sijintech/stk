...
def lineListPlot(data,config,imageName=''):
	...
	lineStyle=[];lineStyle=config['line.style']
	lineLabel=[];lineLabel=config['line.label']
	lineWidth=[];lineWidth=config['line.width']
	markerStyle=[];markerStyle=config['marker.style']
	markerSize=[];markerSize=config['marker.size']
	markerEdgeColor=[];markerEdgeColor=config['marker.edge.color']
	markerEdgeWidth=[];markerEdgeWidth=config['marker.edge.width']
	markerFaceColor=[];markerFaceColor=config['marker.face.color']
	...
	for i in range(0,len(Y)):
		ax.plot(X,Y[i],color=lineColor[i],
...
def lineXYPlot(data,config,imageName=''):
	...
	for i in range(0,len(Y)):
		ax.plot(X[i],Y[i],color=lineColor[i],
					   linestyle=lineStyle[i],
					   linewidth=lineWidth[i],
					   label=lineLabel[i],
					   marker=markerStyle[i],
