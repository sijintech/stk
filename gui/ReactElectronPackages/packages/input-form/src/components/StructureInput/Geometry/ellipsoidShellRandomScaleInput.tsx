...
import type { EllipsoidShellRandomScaleInputData } from "@mupro/typings/GeometryInput";

class EllipsoidShellRandomScaleInput extends React.Component<
  GeometryInputProps,
  {}
> {
  onImport = (geometry: EllipsoidShellRandomScaleInputData) => {
    if (!this.props.formRef.current) return;
    let i = this.props.index;
    console.log("onImport from the EllipsoidShellRandomScale", i, geometry);
    this.props.formRef.current.setFieldsValue({
      ...
      [`geometry${i}_thicknessMin`]: geometry.thicknessMin.trim(),
      [`geometry${i}_thicknessMax`]: geometry.thicknessMax.trim(),
  ...
  onFinish = (values: any): EllipsoidShellRandomScaleInputData => {
    let i = this.props.index;
    console.log("onFinish from the EllipsoidShellRandomScale", i, values);
    return {
      ...
      thicknessMin: values[`geometry${i}_thicknessMin`],
      thicknessMax: values[`geometry${i}_thicknessMax`],
  ...
  render() {
    let i = this.props.index;
    console.log("ellipsoid shell random scale", i);
    return (
      <div>
        ...
        <Form.Item label="Thickness">
          ...
          >
            <Input.Group compact>
              <Form.Item
                name={`geometry${i}_thickMin`}
                noStyle
                rules={[
                  {
                    required: true,
                    message: "lower bound for thickness is required",
                  },
                ]}
              >
                <Input style={{ width: "33%" }} placeholder="2" />
              ...
              <Form.Item
                name={`geometry${i}_thickMax`}
                noStyle
                rules={[
                  {
                    required: true,
                    message: "upper bound for thickness is required",
                  },
                ]}
              >
...
export { EllipsoidShellRandomScaleInput };
