...
type tensorChoices = "11" | "12" | "13" | "22" | "23" | "33";

class Rank2Tensor extends React.Component<
  PropertyInputProps,
  TensorInputState
> {
  ...
  onImport = (tensorIn: TensorInputData) => {
    console.log("Rank2Tensor::onImport", tensorIn, this.props.formRef);
  ...
  applySymmetry = (sym: string, ten: tensorDict): tensorDict => {
    let [_, fields] = this.getSymmetryConstraint(sym);
    let fieldsVal: Record<string, string> = {};
    // let ten = this.state.tensor;
    for (const key in fields) {
      let val = ten[fields[key] as tensorChoices];
      fieldsVal[`${this.state.prefix}_${key}`] = val;
      ten[key as tensorChoices] = val;
    }
    this.props.formRef.current!.setFieldsValue(fieldsVal);
    return ten;
    // this.setState({
  ...
  setZeroDisableComponent = (
    ...
    console.log("set zero disable, ", zeroIds, fieldIds);
  ...
  onSelectTensorSymmetry = (values: string) => {
    console.log("Rank2Tensor::onSelectTenorSymmetry", values);
