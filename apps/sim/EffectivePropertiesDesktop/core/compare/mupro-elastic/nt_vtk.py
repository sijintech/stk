...
class Data(object):
    ...
    def __read_vti_scalar__(self,file_name):
        tree=ET.parse(file_name)
        imageData=tree.find('.//ImageData')
        dimString=imageData.attrib['WholeExtent']
        a,b,c,d,e,f=dimString.split(" ")
        nx=int(b)-int(a)+1
        ny=int(d)-int(c)+1
        nz=int(f)-int(e)+1
        dataArray=tree.findall('.//ImageData//PointData/DataArray')
        indexs=len(dataArray)
        data=np.zeros((nx,ny,nz,indexs))
        count=0
        i=0
        j=0
        k=0
        for index in range(0,indexs):
            dataString=dataArray[index].text
            for word in re.finditer('\s?([^\s]+)\s?',dataString):
                k = int(count/(nx*ny))
                j = int((count-k*nx*ny)/nx)
                i = count-k*nx*ny-j*nx
                count = count + 1
                data[i][j][k][index] = float(word.group(0))
    ...
    def __vti_2_array__(self,file_name):
        if self.data_type == SCALAR:
            data = self.__read_vti_scalar__(file_name)
        else:
            print("The data type is not Scalar, vector, neither domain. Please choose among these three.")
        return data;
    ...
    def __get_array__(self):
        ...
        elif self.__get_file_type__(self.in_file) == ".vti":
            data = self.__vti_2_array__(self.in_file)
        else:
            print("Now only support input of dat and vti file. Please set Data.in_file to be a .dat or .vti file")
            data = 0
        self.data = data
        return data
