    ...
    def analyzeCode(self, curShowCode):
        ...
        if vtk_import:
            ...
            if need_variable is None:
                need_variables = re.finditer(r'(?<!#)\s*(\w+)\s*=\s*vtkRenderer\(\w*\)', curShowCode)
